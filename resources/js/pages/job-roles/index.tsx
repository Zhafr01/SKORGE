...
export default function JobRoleIndex() {
    ...
    useEffect(() => {
        api.get('/job-roles')
            .then((res) => {
                ...
                if (Array.isArray(data) && data.length > 0) setAllRoles(data);
            })
            .catch(() => {});
    }, []);
            ...
            <DialCarouselLayout
                ...
                renderDetail={(item) => (
                    <div className="flex flex-col max-w-2xl">
                        <div className="inline-flex max-w-max items-center px-4 py-2 rounded-full bg-sky-500/10 text-sky-500 mb-6 font-bold text-sm">
                        ...
                        <div className="flex items-center gap-4">
                            <Link
                                ...
                                className="bg-sky-500 hover:bg-sky-400 text-white shadow-[0_10px_30px_rgba(14,165,233,0.3)] hover:shadow-[0_10px_40px_rgba(14,165,233,0.5)] transition-all font-bold text-lg px-8 py-4 rounded-2xl flex items-center gap-2"
                            >
    ...
    return (
        ...
        >
            ...
            <div className="mb-8 rounded-3xl bg-gradient-to-r from-sky-50 dark:from-sky-500/20 to-orange-50 dark:to-orange-500/10 border border-sky-100 dark:border-slate-800 p-8 flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden relative">
                ...
                <Link
                    ...
                    className="relative z-10 shrink-0 inline-flex items-center gap-2 bg-slate-800 dark:bg-slate-800 hover:bg-slate-700 dark:hover:bg-slate-700 text-white font-bold px-6 py-3 rounded-xl transition-all border border-slate-700 hover:border-sky-500/50"
                >
            ...
            <div className="flex flex-wrap gap-2 mb-8">
                {CATEGORIES.map((cat) => (
                    ...
                    >
                        {cat}
                        {cat !== 'All' && (
                            <span className={`ml-2 text-xs px-1.5 py-0.5 rounded ${activeCategory === cat ? 'bg-sky-500/30' : 'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300'}`}>
            ...
            {filtered.length === 0 && (
                <div className="py-20 text-center flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900/50 rounded-3xl border border-slate-200 dark:border-slate-800 border-dashed">
                    ...
                    <button onClick={() => setActiveCategory('All')} className="mt-2 text-sky-600 dark:text-sky-400 text-sm font-medium hover:text-sky-500 dark:hover:text-sky-300 transition-colors">
